# file location  "$([Environment]::GetFolderPath("MyDocuments"))/Powershell/Profile.ps1"
...
function Enter-CyberArkSshSession {
    [CmdletBinding()]
    param (
        [string]$ComputerName
    )
...
$function:fullprompt = {
    ...
    Set-Variable -Name gitBranch -Value (Get-GitBranch) -Scope Script
...
Set-Alias go Enter-PSSession
Set-Alias rdp Enter-RDPSession
Set-Alias cssh Enter-CyberArkSshSession
Set-Alias which Find-ExecLocation
Set-Alias cgit Clone-GitRepo
Set-Alias dgit Download-GitRepo
Set-Alias scs Select-ColorString
Set-Alias tw Trace-Word
...
function prompt {
    ...
    $pythonVenvPath = $env:VIRTUAL_ENV
    if ($pythonVenvPath) {
        $prompt += Write-Prompt "`nvenv:" -ForegroundColor DarkCyan
        $prompt += Write-Prompt " $pythonVenvPath" -ForegroundColor Gray

    }
    $prompt += Write-Prompt "`nps.$psVersion | py.$pyVersion>" -ForegroundColor Cyan
